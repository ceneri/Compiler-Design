...
void astree::print (FILE* outfile, astree* tree, int depth) {
   ...
   fprintf (outfile, "%s \"%s\" (%zd.%zd.%zd) {%zd} ",
    ...
    if (find_ident(tree) != NULL){
        fprintf (outfile, "(%zd.%zd.%zd)",
            tree->origin_filenr, 
            tree->origin_linenr, tree->origin_offset);
    }
    
    //For identifiers, the last number is 
    //the coordinate of the declaration.
    ...
    /*if( tree-> attributes.test( ATTR_variable)){
        ...
        fprintf( outfile, " %zd.%zd.%zd ", 
        tempFilenr, tempLinenr, tempOffset);
    }*/
    
    fprintf( outfile, "\n");
    //prints the information of the next child
